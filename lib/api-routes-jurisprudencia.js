...
router.get('/jurisprudencia/buscar', async (req, res) => {
  try {
    ...
    const resultados = await jurisprudenceSearchService.searchAll(termo, {
 ...
 */
router.get('/jurisprudencia/tribunais', async (req, res) => {
  try {
    const tribunais = Object.entries(datajudService.TRIBUNAIS_DATAJUD).map(([sigla, codigo]) => ({
      sigla,
      codigo,
      nome: sigla // Simplificado: usar sigla como nome
    }));
 ...
 */
router.post('/jurisprudencia/cache/clear', async (req, res) => {
  try {
 ...
 */
router.get('/jurisprudencia/cache/stats', async (req, res) => {
  try {
