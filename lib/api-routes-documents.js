...
// ============================================================
// DOCUMENT CONVERSION ROUTES (Fase 2 - Word por padrão)
// ============================================================

// Lazy import do document converter
let documentConverter = null;

async function getDocumentConverter() {
  if (!documentConverter) {
    try {
      documentConverter = await import('../src/modules/document-converter.js');
      logger.info('[DocumentConverter] Módulo carregado com sucesso');
    } catch (error) {
      logger.error('[DocumentConverter] Erro ao carregar módulo:', error);
      throw new Error('Serviço de conversão de documentos não disponível');
    }
  }
 ...
 *
 * Converte documento Markdown para formato especificado
 */
router.post('/convert', async (req, res) => {
  ...
  try {
    ...
    logger.info(`[DocumentConverter] Convertendo para ${format.toUpperCase()}`, {
      contentLength: content.length,
      title
    });

    // Converter documento
    const converter = await getDocumentConverter();
    const result = await converter.convertDocument(content, format, {
      title,
      author
    });
    ...
    logger.info(`[DocumentConverter] ✅ Conversão concluída`, {
      format,
    ...
    logger.error(`[DocumentConverter] ❌ Erro ao converter`, {
      error: error.message,
      stack: error.stack,
      totalTime: `${totalTime}ms`
    });
 ...
 *
 * Lista formatos de conversão suportados
 */
