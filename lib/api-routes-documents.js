 ...
 *
 * IMPORTANTE: Imports dos serviços são LAZY (carregados apenas quando necessário)
 * para evitar falhas em ambientes onde dependências não estão disponíveis.
 */
...
async function getExtractionService() {
  if (!extractionService) {
    try {
      ...
      throw new Error('Extraction service não está disponível neste ambiente');
    }
  }
  return extractionService;
}

async function getDocumentExtractionService() {
  if (!documentExtractionService) {
    try {
      ...
      throw new Error('Document extraction service não está disponível neste ambiente');
    }
  }
...
router.post('/extraction/extract', async (req, res) => {
  try {
    ...
    const result = await service.extractCompleteDocument({
      filePath,
      processNumber,
      projectName,
      uploadToKB,
