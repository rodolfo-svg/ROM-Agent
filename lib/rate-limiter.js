...
/**
 * Rate limiter geral - 500 requisições por hora por IP
 * AUMENTADO de 100 para 500 para permitir uso intenso do sistema
 */
export const generalLimiter = rateLimit({
  windowMs: 60 * 60 * 1000, // 1 hora
  max: 500, // 500 requisições por hora
  message: {
  ...
  handler: (req, res) => {
    res.status(429).json({
      ...
      message: 'Você excedeu o limite de 500 requisições por hora. Por favor, aguarde antes de tentar novamente.',
      retryAfter: '1 hora'
    });
  }
});
...
/**
 * Rate limiter para API de chat - 60 requisições por minuto por IP
 * AUMENTADO de 10 para 60 para permitir análises exaustivas do KB
 */
export const chatLimiter = rateLimit({
  windowMs: 60 * 1000, // 1 minuto
  max: 60, // 60 requisições por minuto (análises complexas precisam de múltiplas chamadas)
  message: {
