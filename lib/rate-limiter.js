...
const GENERAL_MAX = parseInt(process.env.RATE_LIMIT_GENERAL_MAX || '2000', 10); // Aumentado de 500 para 2000
const CHAT_MAX = parseInt(process.env.RATE_LIMIT_CHAT_MAX || '120', 10); // Aumentado de 60 para 120
...
/**
 * Rate limiter geral - Configurável via env (default: 2000 req/hora)
 * AUMENTADO de 500 para 2000 para evitar bloqueios em uso intenso
...
/**
 * Rate limiter para API de chat - Configurável via env (default: 120 req/min)
 * AUMENTADO de 60 para 120 para streaming rápido e múltiplas mensagens
 */
...
/**
 * Rate limiter para upload de arquivos - 500 requisições por hora
 * ✅ AUMENTADO: Cada upload chunked faz múltiplas requisições (/init + N chunks + /finalize)
 * ✅ Um arquivo de 200MB = ~5 chunks = 7 requisições totais
 * ✅ 500 req/hora permite ~70 uploads de arquivos grandes por hora
 */
export const uploadLimiter = rateLimit({
  windowMs: 60 * 60 * 1000, // 1 hora
  max: 500, // 500 requisições por hora (aumentado de 20)
  skip: defaultSkip,
  message: {
  ...
  handler: (req, res) => {
    res.status(429).json({
      error: 'Limite de uploads excedido',
      message: 'Você excedeu o limite de uploads por hora. Por favor, aguarde antes de enviar mais arquivos.',
      retryAfter: '1 hora',
      tip: 'Considere processar uploads em lotes menores.'
    });
  }
});
...
export function getRateLimitStats() {
  return {
    ...
    upload: { windowMs: 3600000, max: 500, message: '500 req/hora (~70 uploads chunked)' },
    auth: { windowMs: 900000, max: 20, message: '20 tentativas/15min' },
