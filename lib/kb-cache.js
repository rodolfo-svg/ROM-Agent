...
class KBDocumentsCache {
  ...
  async _saveNow() {
    ...
    try {
      ...
      // 🔥 FIX #2: Read-Merge-Write - ler arquivo atual e mesclar mudanças
      let diskDocs = [];
      if (fs.existsSync(this.kbDocsPath)) {
        try {
      ...
      // Criar mapa de documentos do disco por ID
      const diskMap = new Map(diskDocs.map(doc => [doc.id, doc]));

      // Mesclar: adicionar/atualizar documentos do cache local
      ...
      console.log(`💾 KB Cache: Salvo ${this.cache.length} documentos no disco (PID: ${process.pid}, merged)`);
    } catch (error) {
