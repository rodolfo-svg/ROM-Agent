 ...
 *
 * TEMPORÁRIO: Usando ephemeral em produção devido a problema de permissões no /var/data
 * TODO: Após configurar disco persistente no Render Dashboard, reverter para:
 *       export const ACTIVE_PATHS = isRender ? STORAGE_PATHS.persistent : STORAGE_PATHS.ephemeral;
 */
export const ACTIVE_PATHS = STORAGE_PATHS.ephemeral;
...
export const STORAGE_INFO = {
  isRender,
  isPersistent: false, // TEMPORÁRIO: false até configurar /var/data no Render
  environment: isRender ? 'production' : 'development',
  basePath: PROJECT_BASE, // TEMPORÁRIO: usando ephemeral em produção
  diskSize: isRender ? 'ephemeral (temporário - dados perdidos ao reiniciar)' : 'ilimitado (local)'
};
