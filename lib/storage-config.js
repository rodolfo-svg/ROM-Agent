...
const isRender = process.env.RENDER === 'true' ||
                 ...
                 fs.existsSync('/var/data ');  // ← BUG: Render criou com espaço extra!

// Detectar path correto do disco persistente (pode ter espaço extra por bug do Render)
let PERSISTENT_BASE;
if (isRender) {
  if (fs.existsSync('/var/data') && fs.statSync('/var/data').isDirectory()) {
    PERSISTENT_BASE = '/var/data';
    console.log('✅ Disco persistente encontrado em: /var/data');
  } else if (fs.existsSync('/var/data ') && fs.statSync('/var/data ').isDirectory()) {
    PERSISTENT_BASE = '/var/data ';  // ← BUG: tem espaço extra!
    console.warn('⚠️ ATENÇÃO: Disco persistente tem ESPAÇO EXTRA no path: "/var/data "');
    console.warn('⚠️ Corrija no Render Dashboard: delete o disco e recrie como "/var/data"');
  } else {
    PERSISTENT_BASE = '/var/data';  // Fallback
    console.warn('⚠️ /var/data não encontrado, usando fallback');
  }
} else {
  PERSISTENT_BASE = path.join(__dirname, '../var-data-local');
}
...
function getActivePaths() {
  ...
  // No Render, verificar se /var/data está acessível (COM ou SEM espaço extra)
  try {
    // Testar /var/data (correto)
    if (fs.existsSync('/var/data') && fs.statSync('/var/data').isDirectory()) {
      console.log('✅ Disco persistente /var/data detectado');
      return STORAGE_PATHS.persistent;
    }

    // Testar /var/data  (COM ESPAÇO EXTRA - bug do Render Dashboard)
    if (fs.existsSync('/var/data ') && fs.statSync('/var/data ').isDirectory()) {
      console.warn('⚠️ ATENÇÃO: Disco persistente tem ESPAÇO EXTRA: "/var/data "');
      console.warn('⚠️ Sistema vai funcionar, mas corrija no Render Dashboard');
      // Retornar paths com espaço extra
      return {
        base: '/var/data ',
        upload: path.join('/var/data ', 'upload'),
        processed: path.join('/var/data ', 'processed'),
        extracted: path.join('/var/data ', 'extracted'),
        data: path.join('/var/data ', 'data'),
        kb: path.join('/var/data ', 'data', 'knowledge-base'),
        backups: path.join('/var/data ', 'backups'),
        logs: path.join('/var/data ', 'logs'),
        partners: path.join('/var/data ', 'public', 'img', 'partners')
      };
    }
  } catch (err) {
