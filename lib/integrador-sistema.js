...
const DireitoIntertemporal = require('./direito-intertemporal');
const fs = require('fs');
...
class IntegradorSistema {
  constructor() {
    ...
    this.customInstructionsPath = path.join(__dirname, '../config/system_prompts/custom_instructions.md');
    this.contadorUso = 0;
  }
  ...
  async processarRequisicao(requisicao) {
    ...
    if (requisicao.tipoPeca) {
      const promptPath = path.join(__dirname, `../config/system_prompts/${requisicao.tipoPeca}.md`);
