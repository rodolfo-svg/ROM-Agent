 ...
 * - master_admin: Edita prompts globais
...
class PromptsManager {
  ...
  listarPrompts(partnerId, userRole) {
    const prompts = {
      ...
      canEditPartner: userRole === 'partner_admin' || userRole === 'master_admin'
    };
      ...
      try {
        ...
        prompts.partner = partnerFiles.map(file => {
          ...
          return {
            ...
            editable: userRole === 'partner_admin' || userRole === 'master_admin',
            size: content.length,
  ...
  salvarPrompt(promptId, content, partnerId, userRole) {
    ...
    if (!isGlobal && userRole !== 'partner_admin' && userRole !== 'master_admin') {
      throw new Error('Apenas partner_admin pode editar prompts do parceiro');
    }
  ...
  criarOverride(promptId, partnerId, userRole) {
    if (userRole !== 'partner_admin' && userRole !== 'master_admin') {
      throw new Error('Apenas partner_admin pode criar override de prompts');
    }
...
`;
  ...
  removerOverride(promptId, partnerId, userRole) {
    if (userRole !== 'partner_admin' && userRole !== 'master_admin') {
      throw new Error('Apenas partner_admin pode remover override');
    }
