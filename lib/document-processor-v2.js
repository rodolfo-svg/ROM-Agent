...
`;
  ...
  async generateTechnicalFilesBatch(extractedText, documentId, documentName, model = 'sonnet', progressCallback = null) {
    ...
    console.log(`   Modelo: ${MODELS[model].name}`);
    console.log(`   Método: 1 única chamada à IA (otimizado)`);
    ...
    try {
      ...
      const response = await this.analyzeWithPremiumLLM(
        ...
        model,
        'Você é um assistente jurídico especializado. Retorne APENAS o JSON solicitado, sem texto adicional antes ou depois.'
      );
      ...
      try {
        ...
        console.log(`   📄 Primeiros 500 chars da resposta:`, response.analysis.substring(0, 500));
      ...
      for (const fileType of fileTypes) {
        if (analysisData[fileType]) {
          ...
          filesCreated++;
          console.log(`      ✅ ${fileType}.md`);
        } else {
          console.log(`      ⚠️  ${fileType}.md - [NÃO GERADO PELA IA]`);
      ...
      if (saveToKB) {
        ...
        try {
          completeExtractionResult = await extractionPersistenceManager.persistCompleteExtraction(
            ...
            extraction.extractedText,
            {
              pdfPath: options.pdfPath || null,  // Se houver PDF, passar para extrair imagens
              audioFiles: options.audioFiles || [],  // Áudios anexos, se houver
          ...
          console.log(`   🎤 Áudio: ${completeExtractionResult.extractionData.audioCount}`);
