...
`;
    ...
    try {
      ...
      try {
        console.log(`   📏 Tamanho do prompt: ${Math.round(extractionPrompt.length / 1000)}k caracteres`);
        ...
        console.log(`   📦 Resposta recebida do Nova Micro`);
        ...
        console.log(`   ⚠️  Nova Micro falhou: ${novaMicroError.message}`);
        ...
        try {
          ...
          console.log(`   📦 Resposta recebida do Haiku`);
          console.log(`   ✅ sucesso: ${response?.sucesso}`);

          if (response && response.sucesso === false) {
            console.log(`   ❌ Haiku também falhou!`);
            console.log(`   ❌ erro: ${response.erro}`);
            console.log(`   📊 statusCode: ${response.statusCode}`);
