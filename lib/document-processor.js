...
export class DocumentProcessor {
  ...
  async processDirectRead(text, userPrompt, model = 'sonnet', systemPrompt = '') {
    ...
    try {
      ...
      const response = await conversar(messages[0].content, {
...
`;
      ...
      try {
        const response = await conversar(chunkPrompt, {
          modelo: MODELS[model].id,
          systemPrompt: systemPrompt,
          temperature: 0.3,
          maxTokens: 8000,
        ...
        const analysis = response.resposta;
        chunkAnalyses.push({
...
`;
    ...
    try {
      const consolidationResponse = await conversar(consolidationPrompt, {
        modelo: MODELS[model].id,
        systemPrompt: systemPrompt,
        temperature: 0.3,
        maxTokens: 16000,
...
`;
    ...
    try {
      const response = await conversar(analysisPrompt, {
        modelo: MODELS[model].id,
        systemPrompt: systemPrompt,
        temperature: 0.3,
        maxTokens: 16000,
      ...
      const analysisOutputTokens = this.estimateTokens(response.resposta);
