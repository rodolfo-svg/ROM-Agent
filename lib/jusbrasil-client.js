...
export class JusBrasilClient {
  ...
  async search(query, options = {}) {
    ...
    try {
      console.log(`[JusBrasil] Buscando: "${query}"`);
      ...
      console.error('[JusBrasil] Erro na busca:', error.message);

      // Retornar erro amigavel com sugestao de alternativa
      return {
        success: false,
        source: 'jusbrasil',
        error: `Busca no JusBrasil indisponivel: ${error.message}`,
        results: [],
        suggestion: 'Use a ferramenta pesquisar_jurisprudencia que integra Google Search e DataJud como alternativas.',
