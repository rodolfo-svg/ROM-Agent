...
export class GoogleSearchClient {
    ...
    try {
      ...
      if (tribunal) {
        ...
        if (tribunalSite) {
          ...
          // ✅ FIX: Adicionar termos que indicam DECISÃO INDIVIDUAL (não listagem)
          // "inteiro teor" ou "acórdão" indicam página de decisão completa
          // Filtrar URLs de listagem/busca com operadores negativos
          searchQuery = `("inteiro teor" OR "acórdão" OR ementa) ${query} site:${tribunalSite} -inurl:busca -inurl:consulta -inurl:pesquisa -inurl:lista -noticia -taxa -edital`;
          ...
          if (results.length === 0) {
            ...
            usedFallback = true;
            // ✅ FIX: Também aplicar filtros no fallback
            searchQuery = `("inteiro teor" OR ementa) ${query} site:jus.br ${tribunalUpper} -inurl:busca -inurl:consulta -noticia -taxa -edital`;
      ...
      // BUSCA NACIONAL (quando nenhum tribunal especificado)
        ...
        // ✅ FIX: Priorizar páginas de decisão individual, não listagens
        searchQuery = `("inteiro teor" OR "acórdão" OR ementa) ${query} site:jus.br -inurl:busca -inurl:consulta -inurl:pesquisa -noticia -taxa -edital`;
        const response = await this.executeSearch(searchQuery, limit);
        results = this.parseSearchResults(response.data, limit);
      }
  ...
  isListingPage(url) {
    ...
    // Se URL contém algum padrão de listagem, é provável que seja listagem
  ...
  parseSearchResults(data, limit) {
    ...
    const results = data.items.slice(0, limit).map((item, index) => {
      ...
      try {
        ...
        // ✅ NOVO: Rejeitar páginas de listagem/busca (não tem ementa completa)
