...
export class GoogleSearchClient {
  ...
  async search(query, options = {}) {
    ...
    try {
      ...
      if (sites.length > 0 && sites.length <= 5) {
        ...
        if (results.length === 0) {
          ...
          // Ultimo fallback: busca geral
          if (results.length === 0) {
            searchQuery = `jurisprudencia "${query}" tribunal`;
        ...
        // Fallback se ainda 0 resultados
        if (results.length === 0) {
          usedFallback = true;
          searchQuery = `jurisprudencia "${query}"`;
          const fallbackResponse = await this.executeSearch(searchQuery, limit);
          results = this.parseSearchResults(fallbackResponse.data, limit);
        }
      }
