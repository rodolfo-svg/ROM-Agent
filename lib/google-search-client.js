    ...
    try {
      ...
      if (tribunal) {
        ...
        if (tribunalSite) {
          ...
          // Filtrar apenas páginas realmente inúteis (não decisões)
          searchQuery = `(ementa OR "acórdão" OR decisão) ${query} site:${tribunalSite} -noticia -taxa -edital -inurl:login`;
          ...
          if (results.length === 0) {
            ...
            searchQuery = `(ementa OR decisão) ${query} site:jus.br ${tribunalUpper} -noticia -taxa -edital`;
      ...
      // BUSCA NACIONAL (quando nenhum tribunal especificado)
        ...
        // ✅ FIX: Priorizar jurisprudência, evitar apenas páginas administrativas
        searchQuery = `(ementa OR "acórdão" OR decisão) ${query} site:jus.br -noticia -taxa -edital`;
        const response = await this.executeSearch(searchQuery, limit);
        results = this.parseSearchResults(response.data, limit);
      }
