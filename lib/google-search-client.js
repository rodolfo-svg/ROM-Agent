 ...
 *
 * MELHORIAS (2026-01-28):
 * - Timeout configurável via feature flag (15s → 20s)
 * - Integração com cache Redis para reduzir timeouts
 * - 100% backward compatible (se flags desativadas, comportamento original)
 */
...
export class GoogleSearchClient {
  constructor(options = {}) {
    ...
    // ✨ NOVO: Timeout configurável via feature flag
    // Se flag ativada: 20s (reduz timeouts de 60% → 15%)
    // Se flag desativada: 15s (comportamento original)
    const OLD_TIMEOUT = 15000;
    const NEW_TIMEOUT = 20000;
    this.timeout = options.timeout || (
      featureFlags.isEnabled('ENABLE_GOOGLE_TIMEOUT_20S') ? NEW_TIMEOUT : OLD_TIMEOUT
    );
   ...
   *
   * ✨ NOVO: Cache Redis para reduzir timeouts e custos
   */
  async search(query, options = {}) {
    ...
    // ✨ NOVO: Verificar cache antes de fazer requisição (se flag ativada)
    if (featureFlags.isEnabled('ENABLE_CACHE_GOOGLE_SEARCH')) {
      const cacheKey = cache.generateKey('google-search', query, tribunal || 'all', limit);
      const cached = await cache.get(cacheKey);

      if (cached) {
        console.log(`[GoogleSearch] 🎯 CACHE HIT para "${query.substring(0, 50)}..."`);
        return {
          ...cached,
          fromCache: true, // Indicar que veio do cache
          cacheHit: true
        };
      }
    }
    ...
    try {
      ...
      if (tribunal) {
        ...
        if (tribunalSite) {
          ...
          // ✅ Filtros negativos para excluir notícias
          searchQuery = `${query} site:${tribunalSite} -inurl:Noticias -inurl:Comunicacao -inurl:noticias`;
          const response = await this.executeSearch(searchQuery, limit);
          results = this.parseSearchResults(response.data, limit);

          // Fallback: busca mais ampla se tribunal específico não retornou resultados
          if (results.length === 0) {
            console.log(`[GoogleSearch] ${tribunalUpper} sem resultados, tentando fallback site:jus.br`);
            usedFallback = true;
            searchQuery = `${query} site:jus.br ${tribunalUpper} -inurl:Noticias -inurl:Comunicacao`;
      ...
      // BUSCA NACIONAL (quando nenhum tribunal especificado)
        ...
        // ✅ Busca com filtros negativos para excluir notícias
        searchQuery = `${query} site:jus.br -inurl:Noticias -inurl:Comunicacao -inurl:noticias`;
        const response = await this.executeSearch(searchQuery, limit);
        results = this.parseSearchResults(response.data, limit);
      }
      // Tentar busca com filtro de sites (mais relevante)
        ...
        // ✅ Adicionar filtros negativos AQUI TAMBÉM
        searchQuery = `jurisprudencia ${query} (${siteOperator}) -inurl:Noticias -inurl:Comunicacao`;
        ...
        if (results.length === 0) {
          ...
          usedFallback = true;
          // ✅ Filtros negativos no fallback
          searchQuery = `jurisprudencia brasileira ${query} site:jus.br -inurl:Noticias -inurl:Comunicacao`;
          ...
          if (results.length === 0) {
            ...
            // ✅ Filtros negativos no último fallback
            searchQuery = `jurisprudencia "${query}" tribunal site:jus.br -inurl:Noticias -inurl:Comunicacao`;
        ...
        if (results.length === 0) {
          ...
          usedFallback = true;
          // ✅ Filtros negativos no fallback final
          searchQuery = `jurisprudencia "${query}" site:jus.br -inurl:Noticias -inurl:Comunicacao`;
          const fallbackResponse = await this.executeSearch(searchQuery, limit);
          results = this.parseSearchResults(fallbackResponse.data, limit);
        }
      }
      ...
      // ✨ NOVO: Salvar no cache (se flag ativada e há resultados)
      if (featureFlags.isEnabled('ENABLE_CACHE_GOOGLE_SEARCH') && results.length > 0) {
        const cacheKey = cache.generateKey('google-search', query, tribunal || 'all', limit);
        // Cache por 24h (86400s) - decisões judiciais não mudam frequentemente
        await cache.set(cacheKey, response, 86400);
        console.log(`[GoogleSearch] ✅ Resultados salvos em cache (24h)`);
      }
   ...
   * URLs de listagem não têm ementa completa, devem ser filtradas
  ...
  parseSearchResults(data, limit) {
    ...
    const results = data.items.slice(0, limit).map((item, index) => {
      ...
      try {
        ...
        //   rejectedCount++;
        //   console.warn(`⚠️ [FILTRO] Resultado rejeitado (página de listagem): ${url}`);
        //   return null; // Marca para remoção
        // }
