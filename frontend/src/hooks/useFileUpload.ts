...
export function useFileUpload<T = DefaultUploadResponse>(
  ...
  const {
    ...
    allowedExtensions = [],
    timeout = 300000, // 5min (alinhado com server timeout para rotas 'long')
  ...
  /**
   * Executa upload de um arquivo (interno)
   */
      ...
      return new Promise((resolve, reject) => {
        ...
        xhr.upload.onprogress = (event) => {
          if (event.lengthComputable) {
            ...
            const estimatedTimeRemaining = speed > 0 ? remaining / speed : null;
            ...
            setAttachedFiles((prev) =>
              prev.map((af) =>
                af.fileInfo.id === fileId
                  ? { ...af, fileInfo: { ...af.fileInfo, progress } }
                  : af
              )
            );
