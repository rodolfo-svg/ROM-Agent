...
class UsersManager {
  ...
  loadUsers() {
    try {
      if (fs.existsSync(USERS_DATA_FILE)) {
        ...
        // Validar estrutura: garantir que users é um array
    ...
    // Se não existir ou houver erro, criar com estrutura padrão
    const defaultUsers = {
      users: [],
      metadata: {
  ...
  listUsers(includeInactive = false) {
    // Verificação de segurança: garante que users é um array
