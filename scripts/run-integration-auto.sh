...
# Script de Integração Completa - Versão Não-Interativa
# Executa 8 agentes paralelos com modelo Opus automaticamente
...
║          Sistema Multi-Agente Autônomo (AUTO MODE)                  ║
║                                                                      ║
╚══════════════════════════════════════════════════════════════════════╝
EOF

echo ""
echo -e "${GREEN}Iniciando integração completa (modo automático)...${NC}"
echo ""
...
# Verificar variáveis de ambiente críticas (modo informativo, não interativo)
...
if [ ${#MISSING_VARS[@]} -gt 0 ]; then
  ...
  echo -e "${YELLOW}Ver AVISO_IMPORTANTE_CREDENCIAIS.md para detalhes${NC}"
else
...
if [ "$STREAMING" = "true" ]; then
  ...
  echo -e "  ${BLUE}http://localhost:3001/api/integration/progress-stream${NC}"
  echo ""
fi
...
if [ $EXIT_CODE -eq 0 ]; then
  ...
  if [ -f "$LOGS_DIR/tools-status.json" ]; then
    ...
    node -e "
      ...
      try {
        ...
        console.log(\`  ✅ Ferramentas operacionais: \${data.completedTasks}/\${data.totalTasks} (\${data.percentage}%)\`);
        console.log(\`  ⏱️  Tempo total: \${data.duration} minutos\`);
  ...
  echo "  3. Configurar credenciais faltantes (ver AVISO_IMPORTANTE_CREDENCIAIS.md)"
  echo "  4. Re-executar para 100%: ./scripts/run-integration-auto.sh"
  echo ""
else
